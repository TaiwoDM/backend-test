...
const signup = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!fullName || !email || !password)
      return next(
        generateErrorObj("Provide all required details.", 400, "failed")
      );

    if (admin && admin !== false && admin !== true)
      return next(
        generateErrorObj(
          "Incorrect value for admin field. Can only be true or false",
          400,
          "failed"
        )
      );
    ...
    if (userExists)
      return next(
        generateErrorObj(
          "Provided email is already taken. Use another.",
          400,
          "failed"
        )
      );
    ...
    return next(
      generateErrorObj(
        "An error occured when trying to register new user",
        500,
        "error"
      )
    );
