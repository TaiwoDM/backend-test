...
const createDir = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (dirExist) {
      return next(
        generateErrorObj(
          "You already have a folder with that name. Use another name",
          400,
          "failed"
        )
      );
    ...
    return res
      ...
      .json({
        status: "success",
        message: `Directory ${dirName} has been created successfully.`,
      });
  } catch (error) {
    next(
      generateErrorObj(
        "An error occured when trying to create new directory.",
        500,
        "error"
      )
    );
